...
-- supabase/migrations/20260922080200_enforce_idempotency_key_uniqueness.sql
--
-- NON exécuté par cet agent. À exécuter manuellement, uniquement si un
-- problème est constaté après application de PB-02 en production, et
...
--
-- Contenu : la définition EXACTE de create_order actuellement en
-- production (10 arguments), capturée en direct via pg_get_functiondef
-- le 2026-09-22 lors du pré-vol Bloc 5.2 -- PAS une reconstruction
-- approximative. Aucun index n'est touché par ce rollback : PB-02
-- n'en crée aucun (l'index orders_restaurant_idempotency_key existe
-- déjà en dehors de cette migration et n'est pas affecté par elle).
...
--   select prosrc is not null from pg_proc where proname='create_order';
--   -- puis rejouer un appel create_order normal (PICKUP) et confirmer
--   -- qu'il réussit comme avant.
