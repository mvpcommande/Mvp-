-- Enforces restaurants.settings.opening_hours (if set) server-side,
-- so ordering outside business hours is blocked regardless of what
-- the frontend shows. No hours configured for a restaurant = no
-- restriction (backward compatible with existing tenants).
--
-- Format: {"mon": [["11:00","14:30"],["18:30","22:00"]], "tue": [...], ...}
-- Day keys: sun, mon, tue, wed, thu, fri, sat. Missing/empty day = closed.
...
begin
  select settings -> 'opening_hours' into v_opening_hours
