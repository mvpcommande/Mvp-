-- Durcissement anti-abus : coupe-circuit anti-flood par restaurant sur
-- create_order (seul endpoint d'écriture public, appelable par anon).
--
-- Le rate-limit existant par téléphone est contournable (le client fournit
-- le numéro). On ajoute un plafond de rafale PAR RESTAURANT (>= 60 commandes
-- en 2 min => refus) : inoffensif pour un vrai pic, mais casse un flood de
-- spam qui gonflerait la base et noierait le comptoir en temps réel.
--
-- Reste identique par ailleurs à la version 2026-09-06 (prix serveur, bornes
-- pickup_time, horaires, quantité 1..99, idempotency, SumUp, livraison).
-- L'anti-abus par IP/appareil reste à faire au niveau edge (Cloudflare).
