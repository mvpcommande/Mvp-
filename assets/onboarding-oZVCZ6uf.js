import{n as e,t}from"./styles-BSXGE0Lv.js";import{a as n,c as r,d as i,f as a,i as o,l as s,n as c,o as l,r as u,s as d,t as f,u as p}from"./restaurantOwner-D1-TNCgT.js";var m=document.querySelector(`#onboarding-root`),h=[[`pizza`,`Pizza`],[`kebab`,`Kebab`],[`burger`,`Burger`],[`restaurant`,`Restaurant`],[`snack`,`Snack`],[`boulangerie`,`Boulangerie`],[`sushi`,`Sushi`],[`other`,`Autre`]],g=[[`mon`,`Lundi`],[`tue`,`Mardi`],[`wed`,`Mercredi`],[`thu`,`Jeudi`],[`fri`,`Vendredi`],[`sat`,`Samedi`],[`sun`,`Dimanche`]],_=`loading`,v=`signup`,y=``,b=null,x={},S=[];function C(){return`${window.location.protocol}//${window.location.host}`}function w(){return`${C()}/?resto=${b.slug}`}function T(){return`${C()}/admin.html?resto=${b.slug}`}async function E(){if(!e){_=`error`,y=`Supabase n’est pas configuré.`,O();return}let{data:{session:t}}=await e.auth.getSession();if(!t){_=`auth`,O();return}await D()}async function D(){_=`loading`,O();try{if(b=await n(e),!b){_=`create`,O();return}x=b.settings?.opening_hours||{},S=await o(e,b.id),_=`dashboard`}catch(e){console.error(`[FOODATOI onboarding]`,e),y=`Impossible de charger votre espace pour le moment.`,_=`error`}O()}function O(){if(_===`loading`){m.innerHTML=`<div class="onboarding-shell"><p class="eyebrow">FOODATOI</p><h1>Chargement…</h1></div>`;return}if(_===`error`){m.innerHTML=`
  ...
  `,document.querySelector(`#toggle-auth-mode`).onclick=()=>{v=n?`login`:`signup`,y=``,O()},document.querySelector(`#auth-form`).onsubmit=async t=>{t.preventDefault();let r=Object.fromEntries(new FormData(t.currentTarget));if(!l(r.email)){y=`Adresse email invalide.`,O();return}y=``,_=`loading`,O();try{if(n){if((await s(e,r)).pendingConfirmation){_=`auth`,y=`Compte créé ! Vérifiez vos emails pour confirmer votre adresse avant de vous connecter.`,v=`login`,O();return}}else await d(e,r);await D()}catch(e){console.error(`[FOODATOI onboarding]`,e),_=`auth`,y=String(e?.message||``).includes(`Invalid login credentials`)?`Email ou mot de passe incorrect.`:String(e?.message||``).includes(`already registered`)?`Un compte existe déjà avec cet email.`:`Impossible de traiter la demande pour le moment.`,O()}}}function A(){m.innerHTML=`
    <div class="onboarding-shell narrow">
  ...
  `;let n=document.querySelector(`#create-name`),r=document.querySelector(`#create-slug`),i=document.querySelector(`#slug-preview-text`),a=!1;n.oninput=()=>{a||(r.value=p(n.value),i.textContent=r.value||`votre-restaurant`)},r.oninput=()=>{a=!0,r.value=p(r.value),i.textContent=r.value||`votre-restaurant`},document.querySelector(`#create-form`).onsubmit=async t=>{t.preventDefault();let n=Object.fromEntries(new FormData(t.currentTarget));y=``,_=`loading`,O();try{await c(e,n),await D()}catch(e){console.error(`[FOODATOI onboarding]`,e),_=`create`,y=String(e?.message||``).includes(`SLUG_ALREADY_TAKEN`)?`Cette adresse est déjà prise, choisissez-en une autre.`:`Impossible de créer le restaurant pour le moment.`,O()}}}function j(){m.innerHTML=`
    <div class="onboarding-shell">
  ...
  `,M()}function M(){document.querySelector(`#signout-btn`).onclick=async()=>{await r(e),b=null,_=`auth`,v=`login`,O()},document.querySelectorAll(`.copy-btn`).forEach(e=>{e.onclick=()=>{navigator.clipboard.writeText(e.dataset.copy).then(()=>{let t=e.textContent;e.textContent=`Copié ✓`,setTimeout(()=>{e.textContent=t},1500)})}}),document.querySelectorAll(`.hours-row`).forEach(e=>{let t=e.querySelector(`.day-open`),n=e.querySelector(`.hours-inputs`),r=e.querySelector(`.has-r2`),i=e.querySelector(`.r2`);t.onchange=()=>{n.style.display=t.checked?``:`none`},r.onchange=()=>{i.style.display=r.checked?``:`none`}}),document.querySelector(`#hours-form`).onsubmit=async t=>{t.preventDefault();let n={};document.querySelectorAll(`.hours-row`).forEach(e=>{let t=e.dataset.day;if(!e.querySelector(`.day-open`).checked){n[t]=[];return}let r=[[e.querySelector(`.r1-start`).value,e.querySelector(`.r1-end`).value]];e.querySelector(`.has-r2`).checked&&r.push([e.querySelector(`.r2-start`).value,e.querySelector(`.r2-end`).value]),n[t]=r});try{await a(e,b.id,n),x=n;let t=document.querySelector(`#hours-saved`);t.classList.remove(`hidden`),setTimeout(()=>t.classList.add(`hidden`),2e3)}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible d’enregistrer les horaires pour le moment.`)}},document.querySelectorAll(`.product-active`).forEach(t=>{t.onchange=async()=>{try{await i(e,t.dataset.id,t.checked)}catch(e){console.error(`[FOODATOI onboarding]`,e),t.checked=!t.checked}}}),document.querySelectorAll(`[data-delete]`).forEach(t=>{t.onclick=async()=>{if(confirm(`Supprimer ce produit ?`))try{await u(e,t.dataset.delete),S=S.filter(e=>e.id!==t.dataset.delete),O()}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible de supprimer ce produit pour le moment.`)}}}),document.querySelector(`#product-form`).onsubmit=async t=>{t.preventDefault();let n=Object.fromEntries(new FormData(t.currentTarget));try{let t=await f(e,b.id,n);S.push(t),O()}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible d’ajouter ce produit pour le moment.`)}}}E();