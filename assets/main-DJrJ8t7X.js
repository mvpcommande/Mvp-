import{i as e,n as t,r as n,t as r}from"./styles-CFpOEART.js";import"./modulepreload-polyfill-P2Xu9kJm.js";import{d as i,i as a,l as o,n as s,o as c,r as l,s as u,t as d,u as f}from"./restaurantResolver-9IOtyM5e.js";import{t as p}from"./analytics-1YGI5PQu.js";import{a as m,i as h,o as g,r as _}from"./loyalty-Bi6_Ljfe.js";function v(e,t){return[...e,{...t,quantity:t.quantity??1}]}function y(e){return Number(e.reduce((e,t)=>e+t.price*t.quantity,0).toFixed(2))}function b(e,t,n=()=>Date.now()){return{number:`#${n()}`,type:t?.fulfillmentType===`DELIVERY`?`DELIVERY`:`PICKUP`,status:`NEW`,items:e,customer:t,total:y(e),createdAt:new Date().toISOString()}}function ee(e){return typeof e==`string`&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.trim())}async function te(e,t,{email:n,password:r,name:i,phone:a,marketingEmail:o,marketingSms:s}){let{data:c,error:l}=await e.auth.signUp({email:String(n).trim(),password:r});if(l)throw l;let u=c?.user?.id;if(!c?.session)return{pendingConfirmation:!0};let{data:d,error:f}=await e.from(`customers`).insert({restaurant_id:t,auth_user_id:u,name:String(i||``).trim()||null,phone:String(a||``).trim()||null,email:String(n).trim()}).select().single();if(f)throw f;let p=[{channel:`EMAIL`,granted:!!o},{channel:`SMS`,granted:!!s}].map(e=>({restaurant_id:t,customer_id:d.id,channel:e.channel,granted:e.granted,source:`signup`,granted_at:e.granted?new Date().toISOString():null})),{error:m}=await e.from(`marketing_consents`).insert(p);if(m)throw m;return{pendingConfirmation:!1,customer:d}}async function ne(e,{email:t,password:n}){let{data:r,error:i}=await e.auth.signInWithPassword({email:String(t).trim(),password:n});if(i)throw i;return r}async function re(e){let{error:t}=await e.auth.signOut();if(t)throw t}async function ie(e,t){let{data:{user:n}}=await e.auth.getUser();if(!n)return null;let{data:r,error:i}=await e.from(`customers`).select(`*`).eq(`auth_user_id`,n.id).eq(`restaurant_id`,t).maybeSingle();if(i)throw i;return r}async function ae(e,t){let{data:n,error:r}=await e.from(`orders`).select(`id, order_number, status, total_cents, pickup_time, created_at, order_items(product_name, quantity, line_total_cents)`).eq(`customer_id`,t).order(`created_at`,{ascending:!1}).limit(50);if(r)throw r;return n??[]}async function x(e,t){let{data:n,error:r}=await e.from(`marketing_consents`).select(`channel, granted`).eq(`customer_id`,t);if(r)throw r;return n??[]}async function oe(e,{restaurantId:t,customerId:n,channel:r,granted:i}){let{error:a}=await e.from(`marketing_consents`).upsert({restaurant_id:t,customer_id:n,channel:r,granted:i,source:`account`,granted_at:i?new Date().toISOString():null,withdrawn_at:i?null:new Date().toISOString()},{onConflict:`restaurant_id,customer_id,channel`});if(a)throw a;let{error:o}=await e.from(`marketing_consent_events`).insert({restaurant_id:t,customer_id:n,channel:r,granted:i,source:`account`});if(o)throw o}async function se(e){let{error:t}=await e.rpc(`delete_customer_account`);if(t)throw t;await re(e)}var S=null;t(e,{page:`client`,getRestaurantId:()=>S?.id??null});var C=[],w=[],T=null,E=null,D=null,O=`takeaway`,k=`review`,A=`login`,j=``,M=!1,N=null,P=[],F=[],I=null,L=[],R=!1,z=!1,ce=[`Kebab`,`Poulet Paprika`,`Tenders`,`Kefta`,`Merguez`,`Nuggets`,`Steak Haché`,`Cordon Bleu`,`Veggy`],le=[`Ketchup`,`Biggy`,`Marocaine`,`Mayo`,`Blanche`,`Curry`,`Algérienne`,`Harissa`,`Andalouse`,`Brésilienne`,`Moutarde`,`Fromagère`],ue=[`Canette`,`Bouteille`,`Eau`,`Redbull`,`Compote`,`Capri-Sun`],B=document.querySelector(`#root`),V={info:`<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="9"/><line x1="12" y1="11" x2="12" y2="16.5"/><circle cx="12" cy="7.6" r="0.9" fill="currentColor" stroke="none"/></svg>`,account:`<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><circle cx="12" cy="8" r="3.4"/><path d="M5 20c0-3.9 3.1-7 7-7s7 3.1 7 7"/></svg>`,cart:`<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M6 8h12l-1 12H7L6 8Z"/><path d="M9 8V6a3 3 0 0 1 6 0v2"/></svg>`,arrow:`<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="13 6 19 12 13 18"/></svg>`,chevronLeft:`<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><polyline points="15 6 9 12 15 18"/></svg>`};function de(){return!!window.matchMedia?.(`(prefers-reduced-motion: reduce)`).matches}var H=e=>`${Number(e??0).toFixed(2).replace(`.`,`,`)} €`,fe=()=>w.reduce((e,t)=>e+Number(t.quantity??0),0);function pe(){return o(S?.settings?.opening_hours,new Date)}function me(){let e=S?.settings||{},t=Number(e.pickup_eta_min),n=Number(e.pickup_eta_max);if(Number.isFinite(t)&&Number.isFinite(n)&&t>0&&n>=t)return{type:`range`,min:t,max:n};let r=Number(e.pickup_eta_minutes);return Number.isFinite(r)&&r>0?{type:`single`,minutes:r}:null}function he(){let e=me();return e?e.type===`range`?`Retrait ${e.min}–${e.max} min`:`Retrait ~${e.minutes} min`:null}function ge(){let e=me();if(!e)return null;if(e.type===`range`)return`Retrait estimé : ${e.min}–${e.max} min`;let t=new Date(Date.now()+e.minutes*60*1e3);return`Retrait estimé vers ${new Intl.DateTimeFormat(`fr-FR`,{timeZone:`Europe/Paris`,hour:`2-digit`,minute:`2-digit`,hourCycle:`h23`}).format(t)}`}function _e(e){let t=e.options||{};return!!(t.popular||t.is_popular||t.bestseller||t.is_bestseller||t.badge===`populaire`||t.badge===`popular`)}function ve(e){let t=e.options||{};return!!(t.new||t.is_new||t.badge===`nouveau`||t.badge===`new`)}function ye(e){return!!(e.meat||e.sauce||e.drink||Array.isArray(e.options?.groups)&&e.options.groups.length)}function be(){return C.filter(_e).slice(0,4)}function U(){return S?.name||`FOODATOI`}var xe=new Set([`le`,`la`,`les`,`l`,`un`,`une`,`des`,`du`,`de`,`d`,`et`]);function Se(e){let t=String(e||``).trim().split(/\s+/).filter(Boolean);if(!t.length)return``;let n=t.filter(e=>!xe.has(e.toLowerCase().replace(/['’]/g,``)));return(n.length?n:t).slice(0,2).map(e=>e.charAt(0)).join(``).toUpperCase()}function Ce(){return S?.phone||``}function we(){let e=S?.address;return e?typeof e==`string`?e:typeof e==`object`?[e.street,e.postal_code||e.postalCode,e.city].filter(Boolean).join(` · `):``:``}function Te(){return S?.primary_color||`#111111`}async function Ee(){return S=await l(e),console.info(`[FOODATOI] Restaurant résolu:`,S),De(),E=c(e,S.id),S}function De(){S&&(document.documentElement.style.setProperty(`--restaurant-primary`,Te()),document.title=`${U()} · FOODATOI`)}function Oe(e){if(!Array.isArray(e)||!e.length)return null;let t=[...e].sort((e,t)=>e.is_primary===t.is_primary?(e.sort_order??0)-(t.sort_order??0):e.is_primary?-1:1)[0]?.public_url;return t?/^https?:\/\//i.test(t)?t:`/${t.replace(/^\/+/,``)}`:null}function ke(e){let t=e.options&&typeof e.options==`object`?e.options:{},n=Number(e.price_cents??0)/100;return{id:e.id,category:e.category||`Autres`,name:e.name||`Produit`,description:e.description||``,price:n,emoji:t.emoji||t.icon||`🍽️`,imageUrl:Oe(e.product_images),options:t,meat:!!(t.meat||t.meats||t.viande||t.viandes),sauce:!!(t.sauce||t.sauces),drink:!!(t.drink||t.drinks||t.boisson||t.boissons),multipleMeat:!!(t.multipleMeat||t.multiple_meat),tripleMeat:!!(t.tripleMeat||t.triple_meat),createdAt:e.created_at||null}}async function Ae(){if(!e)throw Error(`Supabase n’est pas configuré.`);if(!S?.id)throw Error(`Restaurant non résolu.`);let{data:t,error:n}=await e.from(`products`).select(`
    ...
    `).eq(`restaurant_id`,S.id).eq(`is_active`,!0).order(`sort_order`,{ascending:!0,nullsFirst:!1}).order(`created_at`,{ascending:!0});if(n)throw console.error(`[FOODATOI] Erreur chargement catalogue:`,n),n;return C=(t??[]).filter(e=>e.restaurant_id===S.id).map(ke),console.info(`[FOODATOI] ${C.length} produit(s) chargé(s) pour ${U()}.`),C}function je(){return[`Tous`,...new Set(C.map(e=>e.category).filter(Boolean))]}function Me(e){return String(e).normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).toLowerCase().replace(/[^a-z0-9]+/g,`-`).replace(/(^-|-$)/g,``)}function Ne(e){let t=[],n=new Map;return e.forEach(e=>{let r=e.category||`Autres`;n.has(r)||(n.set(r,[]),t.push(r)),n.get(r).push(e)}),t.map(e=>({category:e,slug:Me(e),items:n.get(e)}))}function Pe(){B.innerHTML=`
  ...
  `}function Fe(t){console.error(`[FOODATOI] Erreur application:`,t),n(e,{context:`main.init`,message:t?.message??String(t),page:`main`}),B.innerHTML=`
    <div class="app-frame client-app">
  ...
  `}function W(){let e=je(),t=U(),n=we(),i=Ce(),a=pe(),o=be(),s=fe(),c=y(w),l=Se(t),d=he();B.innerHTML=`
    <div class="app-frame client-app">
      ...
      <header class="oi-header">
        ...
        <div class="oi-header-row">
          ...
          <div class="brand-lockup">
            ...
            ${S?.logo_url?`<img class="brand-mark-image" src="${r(S.logo_url)}" alt="${r(t)}">`:l?`<span class="brand-mark">${r(l)}</span>`:``}
      ...
      <footer class="site-footer">
        ...
        <div>
          ...
          ${S?.settings?.facebook_url?`
                <a href="${r(S.settings.facebook_url)}" target="_blank" rel="noopener noreferrer">
                  Facebook
                </a>
              `:``}
    ...
    <div class="oi-sticky-cart${w.length?``:` hidden`}" id="sticky-cart-bar">
  ...
  `,ze(),$()}function Ie(e){let t=[];return _e(e)&&t.push(`<span class="oi-badge oi-badge--acid">Populaire</span>`),ve(e)&&t.push(`<span class="oi-badge oi-badge--ink">Nouveau</span>`),`
  ...
  `}function Le(){document.querySelectorAll(`.oi-card-media img, .product-photo`).forEach(e=>{e.onerror=()=>{let t=e.closest(`.oi-card-media`),n=e.closest(`.oi-card`);if(t){t.remove(),n?.classList.add(`oi-card--no-photo`);return}e.remove()}})}function Re(){T&&T.disconnect();let e=document.querySelectorAll(`.menu-category-section`);if(!e.length)return;let t=document.querySelector(`.category-rail`)?.offsetHeight||0;T=new IntersectionObserver(e=>{e.forEach(e=>{if(e.isIntersecting){let t=e.target.id.replace(`menu-cat-`,``);document.querySelectorAll(`[data-category]`).forEach(e=>{let n=e.dataset.target===t;e.classList.toggle(`is-active`,n),n?(e.setAttribute(`aria-current`,`true`),e.scrollIntoView({behavior:de()?`auto`:`smooth`,inline:`center`,block:`nearest`})):e.removeAttribute(`aria-current`)})}})},{rootMargin:`-${t+20}px 0px -70% 0px`,threshold:0}),e.forEach(e=>T.observe(e))}function ze(){document.querySelectorAll(`[data-category]`).forEach(e=>{e.onclick=()=>{let t=e.dataset.target;if(t===`top`){window.scrollTo({top:0,behavior:`smooth`});return}let n=document.getElementById(`menu-cat-${t}`);if(n){let e=document.querySelector(`.category-rail`)?.offsetHeight||0,t=n.getBoundingClientRect().top+window.scrollY-e-12;window.scrollTo({top:t,behavior:`smooth`})}}}),Re(),document.querySelectorAll(`[data-add]`).forEach(e=>{e.onclick=()=>Ue(e.dataset.add)}),document.querySelectorAll(`[data-open]`).forEach(e=>{e.onclick=()=>G(e.dataset.open)}),Le(),document.querySelectorAll(`[data-order-mode]`).forEach(e=>{e.onclick=()=>{O=e.dataset.orderMode,document.querySelectorAll(`[data-order-mode]`).forEach(e=>{let t=e.dataset.orderMode===O;e.classList.toggle(`is-active`,t),e.setAttribute(`aria-pressed`,String(t))})}});let e=document.querySelector(`#open-cart`);e&&(e.onclick=J);let t=document.querySelector(`#sticky-cart-btn`);t&&(t.onclick=J);let n=document.querySelector(`#open-account`);n&&(n.onclick=Ke);let r=document.querySelector(`#open-info`);r&&(r.onclick=Ge);let i=document.querySelector(`#close-cart`);i&&(i.onclick=Y);let a=document.querySelector(`#backdrop`);a&&(a.onclick=Y)}function G(e,t=null){let n=C.find(t=>t.id===e);if(!n)return;let i=t===null?null:w[t],a=i?.options||{},o=Be(n,a),s=Ve(n,a),c=He(n,a),l=Array.isArray(n.options?.groups)?n.options.groups:[],u=Array.isArray(a.groups)?a.groups:[],d=l.map((e,t)=>{let n=e.label||`Choix`,r=(e.min??1)>=1,i=u.find(e=>e.label===n)?.choice??null;return q({name:`grp-${t}`,label:n,options:(e.items||[]).map(String),selected:i,required:r,groupLabel:n})}).join(``),f=Math.max(1,Math.min(20,Number(i?.quantity??1))),p=()=>`${t===null?`Ajouter au panier`:`Enregistrer`} · ${H(n.price*f)}`;document.querySelector(`#modal-content`).innerHTML=`
  ...
  `:``}function Be(e,t={}){if(!e.meat)return``;let n=K(e.options,[`meats`,`meat`,`viandes`,`viande`],ce);return e.tripleMeat?[q({name:`meat-1`,label:`Viande 1`,options:n,selected:t.meat??null}),q({name:`meat-2`,label:`Viande 2`,options:n,selected:t.meat2??null}),q({name:`meat-3`,label:`Viande 3`,options:n,selected:t.meat3??null})].join(``):e.multipleMeat?[q({name:`meat-1`,label:`Viande 1`,options:n,selected:t.meat??null}),q({name:`meat-2`,label:`Viande 2`,options:n,selected:t.meat2??null})].join(``):q({name:`meat-1`,label:`Viande`,options:n,selected:t.meat??null})}function Ve(e,t={}){return e.sauce?q({name:`sauce`,label:`Sauce`,options:K(e.options,[`sauces`,`sauce`],le),selected:t.sauce??null}):``}function He(e,t={}){return e.drink?q({name:`drink`,label:`Boisson`,options:K(e.options,[`drinks`,`drink`,`boissons`,`boisson`],ue),selected:t.drink??null}):``}function J(){k=`review`,document.querySelector(`#drawer`).classList.add(`open`),document.querySelector(`#backdrop`).classList.remove(`hidden`),$()}function Ue(e){let t=C.find(t=>t.id===e);if(t){if(!ye(t)){w=v(w,{...t,quantity:1,options:{}}),W();return}G(e)}}function We(){let e=fe(),t=document.querySelector(`#cart-badge`);t&&(t.textContent=String(e),t.classList.toggle(`hidden`,e===0));let n=document.querySelector(`#open-cart`);n&&n.setAttribute(`aria-label`,`Panier, ${e} article${e>1?`s`:``}`);let r=document.querySelector(`#sticky-cart-bar`);if(r){r.classList.toggle(`hidden`,w.length===0);let t=r.querySelector(`.oi-sticky-cart-count`),n=r.querySelector(`.oi-sticky-cart-total`);t&&(t.textContent=`${e} article${e>1?`s`:``}`),n&&(n.textContent=H(y(w)))}}function Y(){document.querySelector(`#drawer`).classList.remove(`open`),document.querySelector(`#backdrop`).classList.add(`hidden`)}function Ge(){let e=U(),t=we(),n=Ce(),i=u(S?.settings?.opening_hours),a=pe(),o=document.querySelector(`#info-overlay`);o||(o=document.createElement(`div`),o.id=`info-overlay`,o.className=`modal`,document.body.appendChild(o),o.onclick=e=>{e.target===o&&o.remove()}),o.innerHTML=`
  ...
  `,o.querySelector(`#info-close`).onclick=()=>{o.remove()}}async function Ke(){let t=document.querySelector(`#account-overlay`);t||(t=document.createElement(`div`),t.id=`account-overlay`,t.className=`modal`,t.innerHTML=`
    ...
    `,document.body.appendChild(t),t.onclick=e=>{e.target===t&&t.remove()}),j=``,M=!0,Q();try{let{data:{session:t}}=await e.auth.getSession();t?await X():A=`login`}catch(e){console.error(`[FOODATOI] Erreur ouverture compte:`,e),j=`Impossible de charger ton compte pour le moment.`}M=!1,Q()}async function X(){if(N=await ie(e,S.id),!N){A=`login`;return}let[t,n]=await Promise.all([ae(e,N.id),x(e,N.id)]);P=t,F=n;try{if((await _(e,S.id))?.is_active){let[t,n]=await Promise.all([m(e,S.id),h(e,S.id)]);I=t,L=n.filter(e=>e.is_active)}else I=null,L=[]}catch(e){console.error(`[FOODATOI] Erreur chargement fidélité:`,e),I=null,L=[]}A=`dashboard`}function Z(e){return!!F.find(t=>t.channel===e)?.granted}function Q(){let e=document.querySelector(`#account-content`);if(!e)return;let t=j?`<p class="account-error">${r(j)}</p>`:``;if(M){e.innerHTML=`
    ...
    `,Je();return}let n=A===`signup`;e.innerHTML=`
    <button class="modal-close" id="account-close">×</button>
  ...
  `,qe()}function qe(){let t=document.querySelector(`#account-close`);t&&(t.onclick=()=>document.querySelector(`#account-overlay`)?.remove());let r=document.querySelector(`#account-toggle-mode`);r&&(r.onclick=()=>{A=A===`signup`?`login`:`signup`,j=``,Q()});let i=document.querySelector(`#account-form`);i&&(i.onsubmit=async t=>{t.preventDefault();let r=Object.fromEntries(new FormData(t.currentTarget));if(!ee(r.email)){j=`Adresse email invalide.`,Q();return}M=!0,j=``,Q();try{if(A===`signup`){if((await te(e,S.id,r)).pendingConfirmation){M=!1,j=`Compte créé ! Vérifie tes emails pour confirmer ton adresse avant de te connecter.`,A=`login`,Q();return}await X()}else await ne(e,r),await X()}catch(t){console.error(`[FOODATOI] Erreur compte client:`,t),n(e,{restaurantId:S?.id,context:`main.customerAccount`,message:t?.message??String(t),page:`main`}),j=String(t?.message||``).includes(`Invalid login credentials`)?`Email ou mot de passe incorrect.`:String(t?.message||``).includes(`already registered`)?`Un compte existe déjà avec cet email.`:`Impossible de traiter la demande pour le moment.`}M=!1,Q()})}function Je(){let t=document.querySelector(`#account-close`);t&&(t.onclick=()=>document.querySelector(`#account-overlay`)?.remove());let r=document.querySelector(`#account-logout`);r&&(r.onclick=async()=>{await re(e),A=`login`,N=null,P=[],F=[],z=!1,Q()}),document.querySelectorAll(`[data-redeem-reward]`).forEach(t=>{t.onclick=async()=>{if(!R){R=!0,Q();try{await g(e,t.dataset.redeemReward),I=await m(e,S.id),alert(`Récompense échangée ! Montre cet écran en caisse pour en profiter.`)}catch(e){console.error(`[FOODATOI] Erreur échange récompense:`,e),alert(`Impossible d’échanger cette récompense pour le moment.`)}finally{R=!1,Q()}}}}),[`EMAIL`,`SMS`].forEach(t=>{let n=document.querySelector(`#consent-${t.toLowerCase()}`);n&&(n.onchange=async()=>{try{await oe(e,{restaurantId:S.id,customerId:N.id,channel:t,granted:n.checked}),F=await x(e,N.id)}catch(e){console.error(`[FOODATOI] Erreur consentement:`,e),n.checked=!n.checked}})});let i=document.querySelector(`#account-delete`);i&&(i.onclick=()=>{z=!0,Q()});let a=document.querySelector(`#account-delete-cancel`);a&&(a.onclick=()=>{z=!1,Q()});let o=document.querySelector(`#account-delete-confirm`);o&&(o.onclick=async()=>{M=!0,Q();try{await se(e),A=`login`,N=null,P=[],F=[],z=!1,j=`Ton compte et tes données ont été supprimés.`}catch(t){console.error(`[FOODATOI] Erreur suppression compte:`,t),n(e,{restaurantId:S?.id,context:`main.deleteAccount`,message:t?.message??String(t),page:`main`}),j=`Impossible de supprimer le compte pour le moment.`,z=!1}M=!1,Q()})}function $(){let e=document.querySelector(`#cart-content`);if(e){if(!w.length){k=`review`,e.innerHTML=`
      <div class="empty-ticket">
    ...
    `,e.querySelector(`#back-menu`).onclick=Y;return}if(k===`details`){Xe(e);return}Ye(e)}}function Ye(e){let t=y(w),n=ge();e.innerHTML=`
    <div class="oi-cart-items">
      ${w.map((e,t)=>Ze(e,t)).join(``)}
    </div>
  ...
  `,e.querySelectorAll(`[data-remove]`).forEach(e=>{e.onclick=()=>{w.splice(Number(e.dataset.remove),1),$(),We()}}),e.querySelectorAll(`[data-edit]`).forEach(e=>{e.onclick=()=>{G(e.dataset.editId,Number(e.dataset.edit))}});let i=e.querySelector(`#cart-continue`);i&&(i.onclick=()=>{k=`details`,$()})}function Xe(e){let t=y(w);e.innerHTML=`
  ...
  `;let n=e.querySelector(`#cart-back`);n&&(n.onclick=()=>{k=`review`,$()});let i=e.querySelector(`#order-form`);if(i){let t=i.querySelector(`#pickup-date`),n=i.querySelector(`#pickup-time`),a=e.querySelector(`#hours-banner`),s=i.querySelector(`#submit-order`),c=i.querySelectorAll(`input[name="fulfillmentType"]`),l=e.querySelector(`#delivery-address-fields`),u=i.querySelector(`#pickup-date-label span`);function d(){if(!c.length)return;let e=i.querySelector(`input[name="fulfillmentType"]:checked`)?.value===`DELIVERY`;l&&(l.hidden=!e,l.querySelectorAll(`input`).forEach(t=>{t.required=e&&t.name!==`deliveryComplement`})),u&&(u.textContent=e?`JOUR DE LIVRAISON`:`JOUR DE RETRAIT`)}c.forEach(e=>{e.onchange=d}),d();let p=new Date().toLocaleDateString(`en-CA`),m=new Date(Date.now()+5184e6).toLocaleDateString(`en-CA`);t.min=p,t.max=m,t.value=p;function h(){let e=f(n.value,t.value);return e?new Date(e):null}function g(){let e=h(),t=e&&o(S?.settings?.opening_hours,e);s.disabled=!t,a.innerHTML=t?``:`
        ...
        `}t.onchange=g,n.onchange=g,g();let _=i.querySelector(`#email-field`);i.onsubmit=async e=>{e.preventDefault();let t=h();if(!t||!o(S?.settings?.opening_hours,t)){g();return}let n=Object.fromEntries(new FormData(e.currentTarget)),r=String(n.email||``).trim();if(r&&!ee(r)){_?.setCustomValidity(`Adresse email invalide.`),_?.reportValidity();return}_?.setCustomValidity(``);let i=b(w,n),a=[];O===`onsite`&&a.push(`Sur place`),r&&a.push(`Email : ${r}`);let s=String(n.specialInstructions||``).trim();s&&a.push(s),i.notes=a.join(` — `).slice(0,500)||null,D||=crypto.randomUUID(),i.idempotencyKey=D,await $e(i)}}}function Ze(e,t){let n=Qe(e.options);return`
  ...
  `,document.querySelector(`#product-modal`).classList.remove(`hidden`),document.querySelector(`#done`).onclick=()=>{document.querySelector(`#product-modal`).classList.add(`hidden`),W()}}async function tt(){try{p(),Pe(),await Ee(),await Ae(),W()}catch(e){if(!s()&&!d()){window.location.replace(`/pro.html`);return}Fe(e)}}tt();