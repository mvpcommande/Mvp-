...
`)}function m(e,t={},n=(e=``,t=`_blank`)=>window.open(e,t)){let r=n(``,`_blank`);if(!r)return!1;let i=e=>`${(Number(e||0)/100).toFixed(2).replace(`.`,`,`)} €`,a=(e??[]).reduce((e,t)=>e+t.quantity,0),o=(e??[]).reduce((e,t)=>e+t.revenueCents,0),s=(e??[]).map(e=>`
  ...
  `),r.document.close(),r.focus(),r.print(),!0}function h(e,t){if(!t||t<=0)return null;let n=(e??[]).filter(e=>e.status!==`CANCELLED`),r=n.reduce((e,t)=>e+Math.round(Number(t.total??(t.total_cents??0)/100)*100),0);return{orderCount:n.length,totalCents:r,savingsCents:Math.round(r*t)}}function te(e,t,n){if(!e)return null;let r=`caz-food-orders-${Date.now()}`,i=e.channel(r,{config:{broadcast:{self:!1},presence:{key:``}}}).on(`postgres_changes`,{event:`INSERT`,schema:`public`,table:`orders`},e=>{console.log(`[Realtime] Nouvelle commande reçue`,e),t(e)}).on(`postgres_changes`,{event:`UPDATE`,schema:`public`,table:`orders`},e=>{console.log(`[Realtime] Commande mise à jour`,e),t(e)}).on(`postgres_changes`,{event:`DELETE`,schema:`public`,table:`orders`},e=>{console.log(`[Realtime] Commande supprimée`,e),t(e)});return i.subscribe((e,t)=>{console.log(`[Realtime] Statut:`,e),t&&console.error(`[Realtime] Erreur:`,t),e===`SUBSCRIBED`&&console.log(`[Realtime] Abonnement actif pour public.orders`),e===`CHANNEL_ERROR`&&console.error(`[Realtime] CHANNEL_ERROR`,t),e===`TIMED_OUT`&&console.error(`[Realtime] TIMED_OUT`),e===`CLOSED`&&console.warn(`[Realtime] Canal fermé`),n?.(e,t)}),i}function g(e,n=(e=``,t=`_blank`)=>window.open(e,t)){let i=n(``,`_blank`);if(!i)return!1;let a=e.order_items??e.items??[],o=e=>`${(Number(e||0)/100).toFixed(2).replace(`.`,`,`)} €`,s=r(e.pickup_time),c=a.map(e=>`
        <div class="row">
          <div>
            <strong>
...
`&&r.push(10)}return i&&i.type===`pulse`&&(r=r.slice(0,0-this.#e.newline.length)),Uint8Array.from(r)}#f(e,t){if(t===`strict`||this.#e.errors===`strict`)throw Error(e);return console.warn(e),this}static get printerModels(){return Object.entries(A).map((e=>({id:e[0],name:e[1].vendor+` `+e[1].model})))}get columns(){return this.#r.columns}get language(){return this.#e.language}get printerCapabilities(){return this.#i}},le=class{constructor(e){this._events={}}on(e,t){this._events[e]=this._events[e]||[],this._events[e].push(t)}emit(e,...t){let n=this._events[e];n&&n.forEach((e=>{setTimeout((()=>e(...t)),0)}))}},j=[{filters:[{vendorId:1155,productId:22339}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`default`},{filters:[{vendorId:1046,productId:20497}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`zjiang`},{filters:[{vendorId:1155,productId:22592}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`mpt`},{filters:[{vendorId:1049},{vendorId:5380}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`bixolon`},{filters:[{vendorId:1305}],configuration:1,interface:0,language:e=>{let t=`star-line`,n=e.productName;switch(n=n.replace(/^Star\s+/i,``),n=n.replace(/^TSP(1|4|6|7|8|10)(13|43)(.*)?$/,((e,t,n,r)=>`TSP`+t+`00`+(r||``))),n=n.replace(/^TSP(55|65)(1|4)(.*)?$/,((e,t,n,r)=>`TSP`+t+`0`+(r||``))),n=n.replace(/^TSP([0-9]+)(II|III|IV|V|VI)?(.*)?$/,((e,t,n)=>`TSP`+t+(n||``))),n){case`TSP100IV`:case`mPOP`:case`mC-Label3`:case`mC-Print3`:case`mC-Print2`:t=`star-prnt`;break;case`TSP100`:case`TSP100II`:case`TSP100III`:t=`star-graphics`;break;case`BSC10`:case`BSC10BR`:case`BSC10II`:t=`esc-pos`}return t},codepageMapping:`star`},{filters:[{vendorId:1208}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`epson`},{filters:[{vendorId:7568}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`citizen`},{filters:[{vendorId:1497}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`hp`},{filters:[{vendorId:1221}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`epson`},{filters:[{vendorId:4070,productId:33054}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`epson`},{filters:[{vendorId:8137,productId:8214}],configuration:1,interface:0,language:`esc-pos`,codepageMapping:`xprinter`}],M=class{},N=class extends M{#e;#t=null;#n=null;#r={input:null,output:null};constructor(){super(),this.#e=new le,navigator.usb.addEventListener(`disconnect`,(e=>{this.#t==e.device&&this.#e.emit(`disconnected`)}))}async connect(){try{let e=await navigator.usb.requestDevice({filters:j.map((e=>e.filters)).reduce(((e,t)=>e.concat(t)))});e&&await this.#i(e)}catch(e){console.log(`Could not connect! `+e)}}async reconnect(e){let t=await navigator.usb.getDevices(),n=t.find((t=>t.serialNumber==e.serialNumber));n||=t.find((t=>t.vendorId==e.vendorId&&t.productId==e.productId)),n&&await this.#i(n)}async#i(e){this.#t=e,this.#n=j.find((e=>e.filters.some((e=>e.vendorId&&e.productId?e.vendorId==this.#t.vendorId&&e.productId==this.#t.productId:e.vendorId==this.#t.vendorId)))),await this.#t.open(),await this.#t.selectConfiguration(this.#n.configuration),await this.#t.claimInterface(this.#n.interface);let t=this.#t.configuration.interfaces.find((e=>e.interfaceNumber==this.#n.interface));this.#r.output=t.alternate.endpoints.find((e=>e.direction==`out`)),this.#r.input=t.alternate.endpoints.find((e=>e.direction==`in`)),await this.#t.reset(),this.#e.emit(`connected`,{type:`usb`,manufacturerName:this.#t.manufacturerName,productName:this.#t.productName,serialNumber:this.#t.serialNumber,vendorId:this.#t.vendorId,productId:this.#t.productId,language:await this.#a(this.#n.language),codepageMapping:await this.#a(this.#n.codepageMapping)})}async#a(e){return typeof e==`function`?await e(this.#t):e}async listen(){if(this.#r.input)return this.#o(),!0}async#o(){if(this.#t)try{let e=await this.#t.transferIn(this.#r.input.endpointNumber,64);e instanceof USBInTransferResult&&e.data.byteLength&&this.#e.emit(`data`,e.data),this.#o()}catch{}}async disconnect(){this.#t&&(await this.#t.close(),this.#t=null,this.#n=null,this.#e.emit(`disconnected`))}async print(e){if(this.#t&&this.#r.output)try{await this.#t.transferOut(this.#r.output.endpointNumber,e)}catch(e){console.log(e)}}addEventListener(e,t){this.#e.on(e,t)}};function P(e=typeof navigator<`u`?navigator:null){return!!(e&&`usb`in e)}function F(e,t,n={}){let r=e.order_items??e.items??[],i=e=>`${(Number(e||0)/100).toFixed(2).replace(`.`,`,`)} EUR`,a=new t({language:n.language||`esc-pos`,codepageMapping:n.codepageMapping||`epson`});a.initialize().codepage(`auto`).align(`center`).bold(!0).line(n.restaurantName||`FOODATOI`).bold(!1).line(e.order_number??e.number??`—`).align(`left`).rule();for(let e of r){let t=e.product_name??e.name??``,n=e.quantity??1,r=e.line_total_cents??Math.round((e.price||0)*n*100);a.bold(!0).line(`${n}x ${t}`).bold(!1);let o=[];e.options?.meat&&o.push(`Viande: ${e.options.meat}`),e.options?.sauce&&o.push(`Sauce: ${e.options.sauce}`),e.options?.drink&&o.push(`Boisson: ${e.options.drink}`),o.length&&a.line(o.join(` - `)),a.align(`right`).line(i(r)).align(`left`)}a.rule(),e.notes&&a.bold(!0).line(`NOTE:`).bold(!1).line(e.notes).rule();let o=e.total_cents??Math.round((e.total||0)*100);return a.size(2,2).bold(!0).align(`right`).line(`TOTAL ${i(o)}`).bold(!1).size(1,1).align(`left`).newline().newline().cut(),a.encode()}function I(e,t=typeof localStorage<`u`?localStorage:null){let n=new e,r=null,i=`foodatoi-thermal-printer-device`;n.addEventListener(`connected`,e=>{r=e,t&&t.setItem(i,JSON.stringify(e))});function a(){if(!t)return;let e=t.getItem(i);if(e)try{n.reconnect(JSON.parse(e))}catch{}}async function o(){return await n.connect(),r}function s(){return!!r}function c(){return r}function l(e){if(!r)throw Error(`PRINTER_NOT_CONNECTED`);return n.print(e)}return{connect:o,tryAutoReconnect:a,isConnected:s,getDeviceInfo:c,printBytes:l}}var L=document.querySelector(`#admin-root`),R=P()?I(N):null;R&&R.tryAutoReconnect();function z(e,t){if(R?.isConnected())try{let n=F(e,ce,{restaurantName:t,language:R.getDeviceInfo()?.language||`esc-pos`,codepageMapping:R.getDeviceInfo()?.codepageMapping});R.printBytes(n);return}catch(e){console.error(`[FOODATOI admin] Échec impression thermique, repli navigateur:`,e)}g(e)}var B={NEW:`Nouvelle`,ACCEPTED:`Acceptée`,PREPARING:`En préparation`,READY:`Prête`,CANCELLED:`Annulée`},V=null,H=`local`,U=null,W=null,G=null;function K(){return JSON.parse(localStorage.getItem(`caz-food-orders`)||`[]`)}function ue(e){localStorage.setItem(`caz-food-orders`,JSON.stringify(e))}function q(e){return`${Number(e).toFixed(2).replace(`.`,`,`)} €`}async function J(){if(!e){de();return}try{W=await l(e)}catch(e){console.error(`Erreur récupération session:`,e),Z();return}if(W){try{G=await o(e)}catch(t){console.error(`Erreur résolution restaurant:`,t),s(e,{context:`admin.resolveRestaurant`,message:t?.message??String(t),page:`admin`}),X(t);return}V=a(e,G.id),H=`remote`,Y(),await Q();return}Z()}function Y(){U&&e&&e.removeChannel(U),e&&W?.access_token&&e.realtime.setAuth(W.access_token),U=te(e,()=>Q(),t=>{(t===`CLOSED`||t===`TIMED_OUT`||t===`CHANNEL_ERROR`)&&H===`remote`&&(console.warn(`[Realtime] Reconnexion dans 3s...`),s(e,{restaurantId:G?.id,context:`admin.realtime`,message:`Canal realtime perdu (${t}), reconnexion dans 3s`,page:`admin`}),setTimeout(()=>{H===`remote`&&Y()},3e3))})}function de(){L.innerHTML=`
  ...
  `,L.querySelector(`#retry-restaurant`).onclick=()=>J()}function Z(t=``){L.innerHTML=`
    <main class="admin-auth">
      <div class="auth-card">
        <div class="auth-mark">
          CF
  ...
  `;let n=L.querySelector(`#login-form`);n.onsubmit=async t=>{t.preventDefault();let n=new FormData(t.currentTarget),r=t.currentTarget.querySelector(`button`);r.disabled=!0,r.textContent=`CONNEXION…`;try{W=await c(e,n.get(`email`),n.get(`password`))}catch(e){console.error(`Erreur connexion admin:`,e),Z(`Email ou mot de passe incorrect.`);return}try{G=await o(e),V=a(e,G.id),H=`remote`,Y(),await Q()}catch(e){console.error(`Erreur résolution restaurant:`,e),X(e)}}}async function fe(){if(H===`remote`)try{return await V.listOrders()}catch(e){return console.error(`Erreur récupération commandes:`,e),[]}return K()}async function pe(t){let n={NEW:`ACCEPTED`,ACCEPTED:`PREPARING`,PREPARING:`READY`}[t.status];if(n)try{H===`remote`?await V.updateStatus(t.id,n):ue(i(K(),t.id,n)),await Q()}catch(r){console.error(`Erreur changement statut:`,r),s(e,{restaurantId:G?.id,context:`admin.updateStatus`,message:r?.message??String(r),details:{orderId:t.id,from:t.status,to:n},page:`admin`}),alert(`Impossible de modifier le statut de la commande.`)}}async function Q(){if(!W&&H===`remote`){Z();return}let t=(await fe()).slice().sort((e,t)=>new Date(t.created_at??t.createdAt)-new Date(e.created_at??e.createdAt)),n=t.reduce((e,t)=>e+Number(t.total??(t.total_cents??0)/100),0);L.innerHTML=`
    <main class="admin-shell">
      <header class="admin-header">
        <div>
          ...
          <p>
            ${H===`remote`?`Commandes en direct · Supabase Realtime`:`Mode démo local`}
          </p>
        </div>
      ...
      ${(()=>{let e=h(t,G?.settings?.uber_eats_commission_rate);return!e||!e.orderCount?``:`
            <section class="roi-banner">
              ...
              <p>
                ...
                <strong>${q(e.savingsCents/100)}</strong>
      ...
      <p class="admin-note">
        ●
        ${H===`remote`?`Temps réel actif. Les nouvelles commandes apparaissent automatiquement.`:`Mode démo local.`}
      </p>
    </main>
  `;let r=L.querySelector(`#logout`);r&&(r.onclick=async()=>{try{U&&e&&await e.removeChannel(U),e&&await u(e)}finally{W=null,G=null,V=null,H=`local`,U=null,Z()}}),L.querySelectorAll(`[data-next]`).forEach(e=>{e.onclick=()=>{let n=t.find(t=>String(t.id??``)===String(e.dataset.id));n&&pe(n)}}),L.querySelectorAll(`[data-print]`).forEach(e=>{e.onclick=()=>{let n=t.find(t=>String(t.id??``)===String(e.dataset.id));n&&z(n,G?.name)}});let i=L.querySelector(`#export-stock`);i&&(i.onclick=()=>me(t));let a=L.querySelector(`#export-accounting`);a&&(a.onclick=()=>ge(t));let o=L.querySelector(`#connect-printer`);o&&R&&(o.hidden=!1,o.textContent=R.isConnected()?`Imprimante : ${R.getDeviceInfo()?.productName||`connectée`}`:`Connecter l'imprimante`,o.onclick=async()=>{o.textContent=`Connexion…`;try{await R.connect(),o.textContent=`Imprimante : ${R.getDeviceInfo()?.productName||`connectée`}`}catch(e){console.error(`[FOODATOI admin] Connexion imprimante annulée ou échouée:`,e),o.textContent=`Connecter l'imprimante`}});let s=L.querySelector(`#system-health`);s&&(s.onclick=()=>ve());let c=L.querySelector(`#print-stock`);c&&(c.onclick=()=>m(d(t),{rangeLabel:`${t.length} commande${t.length>1?`s`:``} affichée${t.length>1?`s`:``}`})),L.querySelectorAll(`.order-card`).forEach(e=>{e.onclick=n=>{if(n.target.closest(`button`))return;let r=t.find(t=>String(t.id??``)===String(e.dataset.order));r&&_e(r)}})}function me(e){let t=ee(d(e)),n=new Blob([`﻿`+t],{type:`text/csv;charset=utf-8;`}),r=URL.createObjectURL(n),i=document.createElement(`a`);i.href=r,i.download=`caz-food-stock-${new Date().toISOString().slice(0,10)}.csv`,i.click(),URL.revokeObjectURL(r)}function he(e,t,n){let r=p(f(e,t,n)),i=new Blob([`﻿`+r],{type:`text/csv;charset=utf-8;`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`caz-food-comptabilite-${t||n?`${t||`debut`}_${n||`fin`}`:new Date().toISOString().slice(0,10)}.csv`,o.click(),URL.revokeObjectURL(a)}function ge(e){let t=document.createElement(`div`);t.id=`accounting-export-overlay`,t.className=`modal`,t.innerHTML=`
    ...
    <div class="modal-card order-detail-card">
        ...
        <tbody>
          ${n.map(e=>`
                <tr>
                  ...
                  <td class="num">
                    ${q((e.line_total_cents??(e.price??0)*e.quantity*100)/100)}
                  </td>
                </tr>
              `).join(``)}
        </tbody>
      ...
      <div id="detail-timeline">
        ...
        <p class="detail-timeline-loading">
          ${H===`remote`?`Chargement…`:`Non disponible en mode local.`}
        </p>
      </div>
      <button
  ...
  `,document.body.appendChild(a),a.onclick=e=>{e.target===a&&$()},a.querySelector(`#close-order-detail`).onclick=$,a.querySelector(`#print-from-detail`).onclick=()=>z(e,G?.name),H===`remote`&&V?.getOrderEvents)try{let t=await V.getOrderEvents(e.id),n=a.querySelector(`#detail-timeline`);if(!n)return;n.innerHTML=`
        <p class="eyebrow">
          Historique
        </p>
        ${t.length?`
            ...
            `}
      `}catch(e){console.error(`Erreur historique commande:`,e);let t=a.querySelector(`.detail-timeline-loading`);t&&(t.textContent=`Historique indisponible.`)}}async function ve(){let e=document.createElement(`div`);if(e.id=`system-health-overlay`,e.className=`modal`,e.innerHTML=`
    <div class="modal-card order-detail-card">
      <button
        class="modal-close"
  ...
  `,document.body.appendChild(e),e.onclick=t=>{t.target===e&&e.remove()},e.querySelector(`#close-system-health`).onclick=()=>e.remove(),!(H!==`remote`||!V?.getRecentErrors))try{let t=await V.getRecentErrors(),n=e.querySelector(`#health-loading`);if(!n)return;n.outerHTML=t.length?`
        <p>
      ...
      `}catch(t){console.error(`Erreur chargement état système:`,t);let n=e.querySelector(`#health-loading`);n&&(n.textContent=`Impossible de charger les logs.`)}}function ye(e){let t=e.status,i=e.items??e.order_items??[],a={...e.customer??{},name:e.customer?.name??e.customer_name??`Client`,phone:e.customer?.phone??e.customer_phone??`—`,pickupTime:r(e.pickup_time)},o=e.number??e.order_number??`—`,s=e.total??(e.total_cents??0)/100,c=n(t),l=c?`
        <button
          class="primary"
          data-next
          data-id="${e.id}"
