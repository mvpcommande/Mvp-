import{n as e,t}from"./styles-C3YC0o_E.js";import"./modulepreload-polyfill-P2Xu9kJm.js";import{t as n}from"./errorLog-DYeNOlWf.js";import{c as r,i,n as a,r as o,s,t as c}from"./loyalty-Bi6_Ljfe.js";import{a as l,c as u,d,f,h as p,i as m,l as h,m as g,n as _,o as v,p as y,r as b,s as x,t as S,u as C}from"./restaurantOwner-CqwJnIPL.js";function w(e){return e===`logo`?{maxDimension:600,quality:.9}:{maxDimension:1600,quality:.85}}async function T(e,t=`product`){let{maxDimension:n,quality:r}=w(t);if(!e.type.startsWith(`image/`))return e;let i=await createImageBitmap(e),a=Math.min(1,n/Math.max(i.width,i.height)),o=Math.round(i.width*a),s=Math.round(i.height*a),c=document.createElement(`canvas`);c.width=o,c.height=s,c.getContext(`2d`).drawImage(i,0,0,o,s),i.close();let l=await new Promise(e=>c.toBlob(e,`image/jpeg`,r));return!l||l.size>=e.size?e:new File([l],e.name.replace(/\.\w+$/,`.jpg`),{type:`image/jpeg`})}var E=document.querySelector(`#onboarding-root`),D=[[`pizza`,`Pizza`],[`kebab`,`Kebab`],[`burger`,`Burger`],[`restaurant`,`Restaurant`],[`snack`,`Snack`],[`boulangerie`,`Boulangerie`],[`sushi`,`Sushi`],[`other`,`Autre`]],O=[[`mon`,`Lundi`],[`tue`,`Mardi`],[`wed`,`Mercredi`],[`thu`,`Jeudi`],[`fri`,`Vendredi`],[`sat`,`Samedi`],[`sun`,`Dimanche`]],k=`loading`,A=`signup`,j=``,M=null,N={},P=[],F=null,I=[];function L(){return`${window.location.protocol}//${window.location.host}`}function R(){return`${L()}/?resto=${M.slug}`}function z(){return`${L()}/admin.html?resto=${M.slug}`}async function B(){if(!e){k=`error`,j=`Supabase n’est pas configuré.`,H();return}let{data:{session:t}}=await e.auth.getSession();if(!t){k=`auth`,H();return}await V()}async function V(){k=`loading`,H();try{if(M=await l(e),!M){k=`create`,H();return}N=M.settings?.opening_hours||{},P=await m(e,M.id),M.plan!==`commerce`&&(F=await o(e,M.id),I=await i(e,M.id)),k=`dashboard`}catch(e){console.error(`[FOODATOI onboarding]`,e),j=`Impossible de charger votre espace pour le moment.`,k=`error`}H()}function H(){if(k===`loading`){E.innerHTML=`<div class="onboarding-shell"><p class="eyebrow">FOODATOI</p><h1>Chargement…</h1></div>`;return}if(k===`error`){E.innerHTML=`
  ...
  `,document.querySelector(`#toggle-auth-mode`).onclick=()=>{A=n?`login`:`signup`,j=``,H()},document.querySelector(`#auth-form`).onsubmit=async t=>{t.preventDefault();let r=Object.fromEntries(new FormData(t.currentTarget));if(!v(r.email)){j=`Adresse email invalide.`,H();return}j=``,k=`loading`,H();try{if(n){if((await h(e,r)).pendingConfirmation){k=`auth`,j=`Compte créé ! Vérifiez vos emails pour confirmer votre adresse avant de vous connecter.`,A=`login`,H();return}}else await x(e,r);await V()}catch(e){console.error(`[FOODATOI onboarding]`,e),k=`auth`,j=String(e?.message||``).includes(`Invalid login credentials`)?`Email ou mot de passe incorrect.`:String(e?.message||``).includes(`already registered`)?`Un compte existe déjà avec cet email.`:`Impossible de traiter la demande pour le moment.`,H()}}}function W(){E.innerHTML=`
    <div class="onboarding-shell narrow">
  ...
  `;let n=document.querySelector(`#create-name`),r=document.querySelector(`#create-slug`),i=document.querySelector(`#slug-preview-text`),a=!1;n.oninput=()=>{a||(r.value=C(n.value),i.textContent=r.value||`votre-restaurant`)},r.oninput=()=>{a=!0,r.value=C(r.value),i.textContent=r.value||`votre-restaurant`},document.querySelector(`#create-form`).onsubmit=async t=>{t.preventDefault();let n=Object.fromEntries(new FormData(t.currentTarget));j=``,k=`loading`,H();try{await _(e,n),await V()}catch(e){console.error(`[FOODATOI onboarding]`,e),k=`create`,j=String(e?.message||``).includes(`SLUG_ALREADY_TAKEN`)?`Cette adresse est déjà prise, choisissez-en une autre.`:`Impossible de créer le restaurant pour le moment.`,H()}}}function G(){E.innerHTML=`
    <div class="onboarding-shell">
  ...
  `,K()}function K(){document.querySelector(`#signout-btn`).onclick=async()=>{await u(e),M=null,k=`auth`,A=`login`,H()},document.querySelectorAll(`.copy-btn`).forEach(e=>{e.onclick=()=>{navigator.clipboard.writeText(e.dataset.copy).then(()=>{let t=e.textContent;e.textContent=`Copié ✓`,setTimeout(()=>{e.textContent=t},1500)})}}),document.querySelectorAll(`.hours-row`).forEach(e=>{let t=e.querySelector(`.day-open`),n=e.querySelector(`.hours-inputs`),r=e.querySelector(`.has-r2`),i=e.querySelector(`.r2`);t.onchange=()=>{n.style.display=t.checked?``:`none`},r.onchange=()=>{i.style.display=r.checked?``:`none`}}),document.querySelector(`#hours-form`).onsubmit=async t=>{t.preventDefault();let n={};document.querySelectorAll(`.hours-row`).forEach(e=>{let t=e.dataset.day;if(!e.querySelector(`.day-open`).checked){n[t]=[];return}let r=[[e.querySelector(`.r1-start`).value,e.querySelector(`.r1-end`).value]];e.querySelector(`.has-r2`).checked&&r.push([e.querySelector(`.r2-start`).value,e.querySelector(`.r2-end`).value]),n[t]=r});try{await f(e,M.id,n),N=n;let t=document.querySelector(`#hours-saved`);t.classList.remove(`hidden`),setTimeout(()=>t.classList.add(`hidden`),2e3)}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible d’enregistrer les horaires pour le moment.`)}},document.querySelectorAll(`.product-active`).forEach(t=>{t.onchange=async()=>{try{await d(e,t.dataset.id,t.checked)}catch(e){console.error(`[FOODATOI onboarding]`,e),t.checked=!t.checked}}}),document.querySelectorAll(`[data-delete]`).forEach(t=>{t.onclick=async()=>{if(confirm(`Supprimer ce produit ?`))try{await b(e,t.dataset.delete),P=P.filter(e=>e.id!==t.dataset.delete),H()}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible de supprimer ce produit pour le moment.`)}}});let t=document.querySelector(`#color-input`);t&&(t.onchange=async t=>{try{await y(e,M.id,t.target.value),M.primary_color=t.target.value}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible d’enregistrer la couleur pour le moment.`)}});let l=document.querySelector(`#logo-input`);l&&(l.onchange=async t=>{let n=t.target.files[0];if(!n)return;let r=document.querySelector(`#logo-upload-label`),i=r.firstChild.textContent;r.firstChild.textContent=`Envoi…`;try{let t=await T(n,`logo`);M.logo_url=await p(e,M.id,t),H()}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible d’envoyer le logo pour le moment.`),r.firstChild.textContent=i}}),document.querySelectorAll(`.photo-input`).forEach(t=>{t.onchange=async()=>{let r=t.files[0];if(!r)return;let i=t.closest(`.photo-upload-btn`);i.textContent=`Envoi…`;try{let n=await T(r,`product`);await g(e,M.id,t.dataset.product,n),P=await m(e,M.id),H()}catch(t){console.error(`[FOODATOI onboarding]`,t),n(e,{restaurantId:M?.id,context:`onboarding.uploadProductPhoto`,message:t?.message??String(t),page:`onboarding`}),alert(`Impossible d’envoyer cette photo pour le moment.`),i.textContent=`Ajouter une photo`}}}),document.querySelector(`#product-form`).onsubmit=async t=>{t.preventDefault();let r=t.currentTarget,i=Object.fromEntries(new FormData(r)),a=r.photo.files[0];try{let t=await S(e,M.id,i);if(a){let n=await T(a,`product`);await g(e,M.id,t.id,n)}P=await m(e,M.id),H()}catch(t){console.error(`[FOODATOI onboarding]`,t),n(e,{restaurantId:M?.id,context:`onboarding.addProduct`,message:t?.message??String(t),page:`onboarding`}),alert(`Impossible d’ajouter ce produit pour le moment.`)}};let h=document.querySelector(`#save-loyalty-program`);h&&(h.onclick=async()=>{try{await r(e,M.id,{name:document.querySelector(`#loyalty-name`).value,isActive:document.querySelector(`#loyalty-active`).checked,pointsPerEuro:document.querySelector(`#loyalty-rate`).value}),F=await o(e,M.id),H()}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible d’enregistrer le programme pour le moment.`)}});let _=document.querySelector(`#reward-form`);_&&(_.onsubmit=async t=>{t.preventDefault();let n=Object.fromEntries(new FormData(t.currentTarget));try{await c(e,M.id,n),I=await i(e,M.id),H()}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible d’ajouter cette récompense pour le moment.`)}}),document.querySelectorAll(`.reward-active`).forEach(t=>{t.onchange=async()=>{try{await s(e,t.dataset.id,t.checked)}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible de mettre à jour cette récompense pour le moment.`),t.checked=!t.checked}}}),document.querySelectorAll(`[data-delete-reward]`).forEach(t=>{t.onclick=async()=>{try{await a(e,t.dataset.deleteReward),I=I.filter(e=>e.id!==t.dataset.deleteReward),H()}catch(e){console.error(`[FOODATOI onboarding]`,e),alert(`Impossible de supprimer cette récompense pour le moment.`)}}})}B();